...
public class CrcUtil {
    ...
    // Возвращает контрольную сумму CRC-8/SMBUS для length байт из массива data
    ...
    // Возвращает контрольную сумму CRC-8/SMBUS для массива data
    public static byte calculateCRC(byte[] data) {
        return calculateCRC(data, data.length);
    }

    // Возвращает true, если массив data имеет на конце корректную контрольную сумму
    public static boolean verifyCRC(byte[] data) {
        if (data.length == 0) {
            return false;
        }
        
        byte calculatedCRC = calculateCRC(data, data.length - 1);
        return calculatedCRC == data[data.length - 1];
    }

    // Возвращает исходный массив data с добавленой к нему контрольной суммой
